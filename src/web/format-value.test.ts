...
describe("stable measurement formatting", () => {
  it("preserves trailing zeroes without changing normal compact formatting", () => {
    ...
    expect(formatStableAmplitude(10, ChannelUnit.Volts)).toBe("10.000 V");
    expect(formatStableAmplitude(4.5, ChannelUnit.Volts)).toBe("4.500 V");
  });

  it("keeps three decimal places after SI scaling", () => {
    expect(formatStableAmplitude(0.01, ChannelUnit.Volts)).toBe("10.000 mV");
    expect(formatStableHertz(1_000)).toBe("1.000 kHz");
    expect(formatStableSeconds(0.000_001)).toBe("1.000 µs");
  });

  it("never drops the percentage decimal point at magnitude boundaries", () => {
    expect(formatStablePercent(9.5)).toBe("9.500 %");
    expect(formatStablePercent(50)).toBe("50.000 %");
    expect(formatStablePercent(100)).toBe("100.000 %");
  });
});
