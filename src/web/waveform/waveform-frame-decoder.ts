...
function validSource(value: number): value is WaveformSource {
  return value >= WaveformSource.Ch1 && value <= WaveformSource.Math4;
}
...
export function decodeWaveformFrame(buffer: ArrayBuffer): DecodedWaveformFrame {
  if (buffer.byteLength < WAVEFORM_HEADER_BYTES) {
    throw new Error("Waveform frame is shorter than the v2 header");
  }
  const view = new DataView(buffer);
  const magic = view.getUint32(0, true);
  const version = view.getUint8(4);
  const kind = view.getUint8(5);
  const source = view.getUint8(6);
  ...
  if (magic !== WAVEFORM_MAGIC) throw new Error("Invalid waveform magic");
  if (version !== WAVEFORM_FRAME_VERSION) throw new Error(`Unsupported waveform version ${version}`);
  if (headerBytes !== WAVEFORM_HEADER_BYTES) throw new Error(`Unsupported waveform header size ${headerBytes}`);
  if (!validKind(kind)) throw new Error(`Unsupported waveform kind ${kind}`);
  if (!validSource(source)) throw new Error(`Invalid waveform source ${source}`);
  if (kind === WaveformKind.DeepViewport && source > WaveformSource.Ch4) {
    throw new Error("Deep waveform frame must use a physical channel source");
  }
  if (encoding !== WaveformEncoding.IndexedFloat32) {
    throw new Error(`Unsupported waveform encoding ${encoding}`);
  }
  if (!validUnit(unit)) throw new Error(`Unsupported waveform unit ${unit}`);
  if (!(sourceEndSample > sourceStartSample)) throw new Error("Invalid represented source range");
  if (!Number.isFinite(xIncrement) || !Number.isFinite(xOrigin) || !Number.isFinite(xReference)) {
    throw new Error("Waveform X metadata must be finite");
  }
  if (!(xIncrement > 0)) throw new Error("Waveform X increment must be positive");
  for (let offset = 57; offset < WAVEFORM_HEADER_BYTES; offset += 1) {
    if (view.getUint8(offset) !== 0) throw new Error("Waveform v2 reserved header bytes must be zero");
  }

  const expectedBytes = WAVEFORM_HEADER_BYTES + pointCount * WAVEFORM_POINT_BYTES;
  if (buffer.byteLength !== expectedBytes) {
    throw new Error(`Waveform frame length mismatch: expected ${expectedBytes}, got ${buffer.byteLength}`);
  }
  ...
  for (let index = 0; index < pointCount; index += 1) {
    ...
    if (sourceIndex < previousIndex) throw new Error("Waveform source indices must be ordered");
    if (!Number.isFinite(value)) throw new Error("Waveform amplitude must be finite");
