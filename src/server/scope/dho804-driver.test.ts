...
function scriptedDriver(transport: ScriptedTransport): Dho804Driver {
  ...
  const scheduler = {
    ...
    scheduleLatest: <T>(_priority: ScpiPriority, _kind: ScpiOperationKind, execute: ScpiOperation<T>["execute"]) => execute(transport as unknown as ScpiTransport, recorder),
